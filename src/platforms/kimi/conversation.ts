...
const config: PlatformConversationConfig = {
  id: 'kimi',
  name: 'Kimi',
  parserName: 'kimi-parser',
  noMessageError: 'Kimi 当前页面未提取到对话消息。',
  messageSelectors: ['main [class*="chat"]', 'main [class*="message"]', 'main [class*="markdown"]', 'main [class*="segment"]', 'main [class*="bubble"]', '[data-testid*="message"]'],
  userSelectors: ['[class*="user"]', '[class*="question"]'],
  assistantSelectors: ['[class*="assistant"]', '[class*="answer"]', '[class*="markdown"]'],
  titleSelectors: ['h1'],
  titleCleanupPatterns: [/\s*[-|]\s*Kimi\s*$/i]
};

export function exportKimiCurrentConversation() {
  return exportPlatformCurrentConversation(config);
}

export function diagnoseKimiCurrentPage() {
